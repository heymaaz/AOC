...
public class Fencing {
    ...
    public static void main(String[] args) {
        ...
        List<List<Character>> input = Input.readFile("TwentyFour/Day12/example.txt");
        boolean[][] visitedA = new boolean[input.size()][input.get(0).size()];
        long Fencing = 0l;
        for(int i = 0; i<input.size(); i++) {
            for(int j = 0; j<input.get(i).size(); j++) {
                ...
                long A = recursiveArea(input, currentPlant, i, j, visitedA);
                boolean[][] visitedP = new boolean[input.size()][input.get(0).size()];
                long P = recursivePerimeter(input, currentPlant, i, j, visitedP);
                
                System.out.println("A region of "+currentPlant+" plants with price "+A+" * "+P+" = "+(A*P)+".");
                Fencing+=A*P;
            }
        }
    ...
    static long recursiveArea(List<List<Character>> input, char currentPlant, int i, int j, boolean[][] visited) {
        if(i<0 || j<0 || i>=input.size() || j>=input.get(i).size() || input.get(i).get(j)!=currentPlant || visited[i][j])
            return 0;
        visited[i][j]=true;
        ...
        for (int d = 0; d < DIRECTIONS.length; d++) {
            ...
            area += recursiveArea(input, currentPlant, newRow, newCol, visited);
        }
        return area;
    }
    
    static long recursivePerimeter(List<List<Character>> input, char currentPlant, int i, int j, boolean[][] visited) {
        if(i<0 || j<0 || i>=input.size() || j>=input.get(i).size() || input.get(i).get(j)!=currentPlant)
            return 1;
        if(visited[i][j])
            return 0;
        visited[i][j]=true;
        ...
        for (int d = 0; d < DIRECTIONS.length; d++) {
            int newRow = i + DIRECTIONS[d][0];
            int newCol = j + DIRECTIONS[d][1];
            perimeter += recursivePerimeter(input, currentPlant, newRow, newCol, visited);
        }
        return perimeter;
    }
